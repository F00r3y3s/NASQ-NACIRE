...
- Add a dedicated public `/auth` route for email sign-in and sign-up, and preserve the user’s intended protected destination through a validated `next` redirect instead of sending anonymous users back to the dashboard.
- Keep public discovery open for guests, but route protected entry points such as submit, publish, relay response, and account access through auth-first navigation so the artifact CTAs remain usable before membership verification.
- Support an environment-gated demo-content mode through `NASQ_ENABLE_DEMO_DATA`, and use it only as a public-read fallback when showcase builds need client-ready records before live platform data exists.
- Treat “sign-up works” as more than raw auth creation: brand-new authenticated users without memberships must land in a first-run account onboarding flow that creates a private company profile and a pending `company_admin` membership request for admin verification.
- Keep self-serve onboarding constrained to one pending company membership request per new account in v1, and route all later verification or company corrections through the admin governance console instead of widening public write access.
- Rebuild the public analytics route around the artifact’s exact panel inventory for demo fidelity, even when some metrics must be derived from the existing public-safe snapshot instead of coming from dedicated analytics tables.
- Keep the analytics data source singular: derive alert chips, live-user estimates, weekly sessions, sector donut, and traffic-source rows inside `src/lib/data/public-analytics.ts` from the same public-safe challenge, solution, company, sector, and signal records rather than introducing a second demo-only data pipeline.
